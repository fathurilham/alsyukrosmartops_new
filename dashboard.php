...
<header class="topbar">
    <button id="menuToggle" style="display:none;background:none;border:none;font-size:22px;cursor:pointer;margin-right:6px;"><i data-lucide="menu"></i></button>
    <div class="topbar-title"><?= htmlspecialchars($pageTitle) ?><span><?= $dayName ?>, <?= $today ?></span></div>
    
    <div class="topbar-search" style="margin-right:auto; margin-left: 15px; display:flex; align-items:center; flex: 1; max-width: 250px;">
        <div class="search-input-wrap" style="width: 100%;">
            <i data-lucide="search" class="search-icon"></i>
            <input type="text" placeholder="Cari..." style="width: 100%;">
        </div>
    </div>
    ...
    <div class="topbar-actions">
        <div style="position:relative;">
            ...
            $notifCount = $totalNotifItems;
            ?>
            <button class="icon-btn" id="notifBtn"><i data-lucide="bell" style="width:20px;height:20px;"></i><?php if($notifCount>0): ?><span class="badge"><?= $notifCount ?></span><?php endif; ?></button>
            <div class="notif-dropdown" id="notifDropdown">
                <div class="notif-header"><strong><i data-lucide="bell" style="width:16px;height:16px;vertical-align:-3px;margin-right:4px;"></i> Notifikasi</strong><a href="dashboard.php?mod=notifikasi">Lihat semua</a></div>
                <div class="notif-list">
                    <?php if($notifCount > 0): ?>
                        <?php foreach($realNotifs as $n): ?>
                        <div class="notif-item <?= $n[3] ?>" onclick="location.href='<?= $n[4] ?? '#' ?>'" style="cursor:pointer;"><div class="notif-dot" style="<?= $n[3]==='read'?'background:#ddd;':'' ?>"></div><div class="notif-content"><p><i data-lucide="<?= $n[0] ?>" style="width:14px;height:14px;margin-right:6px;vertical-align:-2px;"></i> <?= $n[1] ?></p><span><?= $n[2] ?></span></div></div>
                        <?php endforeach; ?>
                    <?php else: ?>
        ...
        <a href="dashboard.php?mod=profile" class="icon-btn"><i data-lucide="user" style="width:20px;height:20px;"></i></a>
        <a href="#" class="icon-btn" onclick="openModal('modalLogout')"><i data-lucide="log-out" style="width:20px;height:20px;"></i></a>
    </div>
</header>
...
<a href="<?= $qa[1] ?>" class="qa-btn"><span class="qa-icon"><i data-lucide="<?= $qa[0] ?>"></i></span><span class="qa-label"><?= $qa[2] ?></span></a>
<?php endforeach; ?>
</div>
<div class="grid-2">
    <div class="card">
        <div class="card-header"><div class="card-title"><i data-lucide="clipboard-list"></i> Aktivitas Terbaru</div></div>
        <div class="card-body" style="padding:0;">
            ...
            if ($resInv) while($r = $resInv->fetch_assoc()) $tls[] = ['package', 'eafaf1', 'Penambahan barang <strong>'.htmlspecialchars($r['nama']).'</strong>', $r['tgl_beli'] ?: date('Y-m-d'), strtotime($r['tgl_beli']?:date('Y-m-d'))];
            
            // 2. Peminjaman
            $resPjm = $conn->query("SELECT peminjam, barang, tgl_pinjam FROM peminjaman ORDER BY id DESC LIMIT 2");
            if ($resPjm) while($r = $resPjm->fetch_assoc()) $tls[] = ['refresh-cw', 'fef9e7', 'Peminjaman <strong>'.htmlspecialchars($r['barang']).'</strong> oleh '.$r['peminjam'], $r['tgl_pinjam'] ?: date('Y-m-d'), strtotime($r['tgl_pinjam']?:date('Y-m-d'))];
            
            // 3. Request Maintenance
            $resMnt = $conn->query("SELECT fasilitas, tgl_request FROM request_maint ORDER BY id DESC LIMIT 2");
            if ($resMnt) while($r = $resMnt->fetch_assoc()) $tls[] = ['alert-triangle', 'fdedec', 'Request perbaikan <strong>'.htmlspecialchars($r['fasilitas']).'</strong>', $r['tgl_request'] ?: date('Y-m-d'), strtotime($r['tgl_request']?:date('Y-m-d'))];
            
            // 4. Arsip
            $resArs = $conn->query("SELECT nama, tgl_upload FROM arsip ORDER BY id DESC LIMIT 2");
            if ($resArs) while($r = $resArs->fetch_assoc()) $tls[] = ['folder', 'f0f5f2', 'Arsip <strong>'.htmlspecialchars($r['nama']).'</strong> diunggah', $r['tgl_upload'] ?: date('Y-m-d'), strtotime($r['tgl_upload']?:date('Y-m-d'))];
            
            // 5. Event
            $resEv = $conn->query("SELECT nama, tgl_mulai FROM event ORDER BY id DESC LIMIT 2");
            if ($resEv) while($r = $resEv->fetch_assoc()) $tls[] = ['tent', 'f4ecf7', 'Event <strong>'.htmlspecialchars($r['nama']).'</strong>', $r['tgl_mulai'] ?: date('Y-m-d'), strtotime($r['tgl_mulai']?:date('Y-m-d'))];
            ...
            if (!empty($tls)):
                ...
                <div class="tl-item" style="padding:12px 20px;">
                    <div class="tl-dot" style="background:#<?= $t[1] ?>;"><i data-lucide="<?= $t[0] ?>"></i></div>
    ...
    <div class="card">
        ...
        <div class="card-body" style="padding:0;">
            ...
            if ($resLatePjm && ($c = $resLatePjm->fetch_assoc()['cnt']) > 0) $als[] = ['alert-circle', "$c peminjaman terlambat dikembalikan", 'Inventaris'];
            
            $resMntReq = $conn->query("SELECT count(*) as cnt FROM request_maint WHERE status='menunggu'");
            if ($resMntReq && ($c = $resMntReq->fetch_assoc()['cnt']) > 0) $als[] = ['alert-circle', "$c request maintenance belum diproses", 'Maintenance'];
            
            $resEvPrep = $conn->query("SELECT count(*) as cnt FROM event WHERE status='persiapan'");
            if ($resEvPrep && ($c = $resEvPrep->fetch_assoc()['cnt']) > 0) $als[] = ['info', "$c event masih dalam tahap persiapan", 'Event'];
            
            $resInvStok = $conn->query("SELECT count(*) as cnt FROM inventory WHERE stok = 0");
            if ($resInvStok && ($c = $resInvStok->fetch_assoc()['cnt']) > 0) $als[] = ['alert-circle', "$c barang kehabisan stok", 'Inventaris'];
            
            if (empty($als)) $als[] = ['check-circle', 'Semua sistem berjalan dengan baik', 'Sistem'];
            
            foreach($als as $a): ?>
            <div style="padding:11px 20px;border-bottom:1px solid #f0f5f2;display:flex;gap:10px;align-items:center;">
                <i data-lucide="<?= $a[0] ?>" style="color:var(--danger)"></i>
...
<div class="tab-nav-bar">
    ...
    <a href="?mod=aktivitas&sub=monitoring_aktivitas" class="tab-link <?= $aktSub==='monitoring_aktivitas'?'active':'' ?>"><i data-lucide="file-signature"></i> Monitoring Aktivitas</a>
    <?php else: ?>
    <a href="?mod=aktivitas&sub=input_aktivitas"      class="tab-link <?= $aktSub==='input_aktivitas'?'active':'' ?>"><i data-lucide="pencil"></i> Input Aktivitas Harian</a>
    <a href="?mod=aktivitas&sub=riwayat_aktivitas"    class="tab-link <?= $aktSub==='riwayat_aktivitas'?'active':'' ?>"><i data-lucide="clipboard-list"></i> Riwayat Aktivitas</a>
    <?php endif; ?>
</div>
<?php if($aktSub==='input_aktivitas'): ?>
<div class="grid-2">
    <div class="card">
        <div class="card-header"><div class="card-title"><i data-lucide="pencil"></i> Input Aktivitas Harian</div></div>
        <div class="card-body">
            ...
            <button class="btn btn-success w-full" onclick="simpanAktivitas()"><i data-lucide="save"></i> Simpan Aktivitas</button>
        </div>
    </div>
    <div class="card">
        <div class="card-header"><div class="card-title"><i data-lucide="clipboard-list"></i> Aktivitas Saya Terbaru</div></div>
        <div id="listAktivitas" style="padding:0;"></div>
    </div>
</div>
<?php elseif($aktSub==='riwayat_aktivitas'||$aktSub==='monitoring_aktivitas'): ?>
<div class="card">
    <div class="card-header"><div class="card-title"><i data-lucide="file-signature"></i> <?= $aktSub==='monitoring_aktivitas'?'Monitoring':'Riwayat' ?> Aktivitas</div>
...
<div class="card">
    <div class="card-header">
        <div class="card-title"><i data-lucide="bell"></i> Semua Notifikasi</div>
        <button class="btn btn-outline btn-sm" onclick="toast('Semua notifikasi ditandai sudah dibaca','success')"><i data-lucide="check-circle"></i> Tandai Semua Dibaca</button>
    </div>
...
<div class="card">
    <div class="card-header">
        <div class="card-title"><i data-lucide="key"></i> Kelola Pengguna Sistem</div>
        <button class="btn btn-success" onclick="openModal('modalTambahUser')"><i data-lucide="plus-circle"></i> Tambah User</button>
    </div>
    <div class="table-wrap">
        <table>
            ...
            <tr>
                ...
                <td>
                    <div class="flex gap-2">
                        <button class="btn btn-outline btn-sm" onclick="editUser(<?= htmlspecialchars(json_encode($u)) ?>)"><i data-lucide="pencil"></i> Edit</button>
...
<div class="modal-overlay" id="modalTambahUser">
    <div class="modal">
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-success" onclick="simpanUser()"><i data-lucide="save"></i> Simpan</button>
        </div>
    </div>
</div>
<script>
...
<div style="background:linear-gradient(135deg,#0f4525,#2d9b5a);border-radius:16px;padding:32px;color:white;margin-bottom:24px;display:flex;align-items:center;gap:24px;flex-wrap:wrap;box-shadow:0 8px 32px rgba(15,69,37,.2);">
    ...
    <div>
        ...
        <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;">
            ...
            <span style="background:rgba(255,255,255,.15);padding:4px 12px;border-radius:20px;font-size:12px;"><i data-lucide="calendar"></i> Bergabung <?= $user['created_at']??date('Y-m-d') ?></span>
        </div>
    </div>
...
<div class="grid-2">
    <div class="card">
        <div class="card-header"><div class="card-title"><i data-lucide="user"></i> Edit Profil</div></div>
        <div class="card-body">
            ...
            <button class="btn btn-success w-full" onclick="toast('✅ Profil berhasil diperbarui!','success')"><i data-lucide="save"></i> Simpan Perubahan</button>
        </div>
    </div>
    <div>
        <div class="card" style="margin-bottom:18px;">
            ...
            <div class="card-body">
                ...
                <button class="btn btn-warning w-full" onclick="ubahPassword()"><i data-lucide="key"></i> Ubah Password</button>
            </div>
        </div>
        <div class="card">
...
<div class="modal-overlay" id="modalLogout">
    <div class="modal" style="max-width: 400px; text-align: center;">
        <div class="modal-body" style="padding: 30px;">
            <div style="font-size: 48px; margin-bottom: 16px;"><i data-lucide="log-out"></i></div>
...
// Mobile sidebar toggle dipindahkan ke js/main.js
</script>
</body>
</html>
